...
import { VOLLZANBEL, JUDRADJIM, GRAUSAMKEIT, MANA_SHIELD, DETECT_MAGIC } from '../techniques/SharedTechniques';
...
const serieAbility: Ability = {
	...
	abilityStartOfTurnEffect: (character: Character, battle: Battle) => {
		...
		if (serieChar.toyingNextTurn) {
			battle.addToBattleLog(`${character.name} acts aloof.`);
...
const Serie = new CharacterData({
	...
	cosmetic: {
		...
		color: 0xe8b961,
		description:
			'The Great Mage Serie, founder of the Continental Magic Association. An ancient elf with immense magical power who often toys with opponents.'
	},
	level: 70,
	races: [Race.Elf],
	baseStats: serieStats,
	techniques: [VOLLZANBEL, JUDRADJIM, GRAUSAMKEIT, MANA_SHIELD, DETECT_MAGIC],
